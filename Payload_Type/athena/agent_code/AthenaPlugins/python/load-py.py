...
class LoadModuleCommand(CommandBase):
    ...
    async def create_tasking(self, task: MythicTask) -> MythicTask:
        ...
        if(module == "ds"):
            ...
            resp = await MythicRPC().execute("create_subtask_group", tasks=[
                {"command": "load-assembly", "params" : {"libraryname":"Microsoft.Scripting.dll", "target":task.args.get_arg('target').lower()}},
                {"command": "load-assembly", "params" : {"libraryname":"IronPython.Modules.dll", "target":task.args.get_arg('target').lower()}},
                {"command": "load-assembly", "params" : {"libraryname":"IronPython.dll", "target":task.args.get_arg('target').lower()}},
                {"command": "load-assembly", "params" : {"libraryname":"System.CodeDom.dll", "target":task.args.get_arg('target').lower()}},
                {"command": "load-assembly", "params" : {"libraryname":"Microsoft.Dynamic.dll", "target":task.args.get_arg('target').lower()}},
                {"command": "load", "params" : {"command":"python"}}
                ],
                subtask_group_name = "python", group_callback_function=self.load_completed.__name__, parent_task_id=task.id)
